...
import { Component, AfterViewInit, Inject, PLATFORM_ID, ElementRef, ViewChild, OnDestroy, Renderer2 } from '@angular/core';
...
export class PostDetailComponent implements AfterViewInit, OnDestroy {
  ...
  @ViewChild('replyForm', { read: ElementRef, static: false }) replyFormEl: ElementRef;
  ...
  public isReplyFormVisible = false;
  ...
  ngAfterViewInit() {
    ...
    this.route.paramMap.pipe(
      ...
      switchMap((params: ParamMap) => {
        ...
        const extensionPos = slug.lastIndexOf('.html');
        if (extensionPos > 0) {
      ...
      .subscribe(post => {
        if (post) {
          ...
          //add meta data
          for (const key in this.post.meta) {
            if (key.indexOf('og:') == 0) {
